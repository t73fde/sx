...
func executeTestCases(t *testing.T, testcases []testcase) {
	for _, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			...
			child.Bind(sf.MustMake("x"), sx.Int64(3))
			child.Bind(sf.MustMake("y"), sx.Int64(5))
			child.Bind(sf.MustMake("lang0"), sx.MakeString(""))
			child.Bind(sf.MustMake("lang1"), sx.MakeString("de-DE"))
...
func TestQuasiQuoteExt(t *testing.T) {
	...
	executeTestCases(t, []testcase{
		...
		{src: "(quasiquote (0 (unquote-splicing 1) 4))", exp: "argument 2 is not a list, but sx.Int64/1", withErr: true},
