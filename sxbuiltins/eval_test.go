...
var tcsEval = tTestCases{
	...
	{name: "err-execute-expression-2-bind",
		...
		exp:     "{[{execute-expression: argument 2 is not a binding, but sx.Int64/2}]}",
		withErr: true,
	},
	{name: "execute-expression-one", src: "(execute-expression (parse-expression '1))", exp: "1"},
	{name: "execute-expression-two", src: "(execute-expression (parse-expression '1)  (current-binding))", exp: "1"},
