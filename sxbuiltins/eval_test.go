...
var tcsEval = tTestCases{
	...
	{name: "err-parse-expression-2",
		...
		exp:     "{[{parse-expression: argument 2 is not a binding, but sx.Int64/3}]}",
		withErr: true,
	},
	{name: "err-parse-expression-err-parse-2",
		src:     "(parse-expression '(set!) (current-binding))",
		exp:     "{[{set!: need at least two arguments}]}",
		withErr: true,
	},
	{name: "parse-expression-one", src: "(parse-expression 1)", exp: "#<{OBJ 1}>"},
	{name: "parse-expression-one-2", src: "(parse-expression 1 (current-binding))", exp: "#<{OBJ 1}>"},
	...
	{name: "err-run-expression-0",
		src:     "(run-expression)",
		exp:     "{[{run-expression: between 1 and 2 arguments required, but none given}]}",
		withErr: true,
	},
	{name: "err-run-expression-3",
		src:     "(run-expression 1 2 3)",
		exp:     "{[{run-expression: between 1 and 2 arguments required, but 3 given: [1 2 3]}]}",
		withErr: true,
	},
	{name: "err-run-expression-1",
		src:     "(run-expression 1)",
		exp:     "{[{run-expression: argument 1 is not an expression, but sx.Int64/1}]}",
		withErr: true,
	},
	{name: "err-run-expression-2",
		src:     "(run-expression 1 2)",
		exp:     "{[{run-expression: argument 1 is not an expression, but sx.Int64/1}]}",
		withErr: true,
	},
	{name: "err-run-expression-2-bind",
		src:     "(run-expression (parse-expression '1) 2)",
		exp:     "{[{run-expression: argument 2 is not a binding, but sx.Int64/2}]}",
		withErr: true,
	},
	{name: "run-expression-one", src: "(run-expression (parse-expression '1))", exp: "1"},
	{name: "run-expression-two", src: "(run-expression (parse-expression '1)  (current-binding))", exp: "1"},
	...
	{name: "err-eval-2",
		src:     "(eval '(set!) 2)",
		exp:     "{[{eval: argument 2 is not a binding, but sx.Int64/2}]}",
		withErr: true,
	},
	{name: "err-eval-err-parse",
		src:     "(eval '(set!) (current-binding))",
		exp:     "{[{set!: need at least two arguments}]}",
		withErr: true,
	},
	{name: "eval-one", src: "(eval 1)", exp: "1"},
	{name: "eval-one-parse", src: "(eval (parse-expression 1))", exp: "1"},
	{name: "eval-two", src: "(eval 1 (current-binding))", exp: "1"},
}
