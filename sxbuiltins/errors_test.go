...
var tcsError = tTestCases{
	...
	{name: "err-not-bound-error-nosy-2",
		src:     "(not-bound-error 1 2)",
		exp:     "{[{argument 1 is not a symbol, but sx.Int64/1}]}",
		withErr: true,
	},
	{name: "not-bound-error-sym",
		src:     "(not-bound-error 'sym)",
		exp:     "{[{symbol \"sym\" not bound in \"not-bound-error-sym\"->\"vars\"->\"root\"}]}",
		withErr: true,
	},
	{name: "not-bound-error-sym-noenv",
		src:     "(not-bound-error 'sym 1)",
		exp:     "{[{argument 2 is not a frame, but sx.Int64/1}]}",
		withErr: true,
	},
	{name: "not-bound-error-sym-env",
		src:     "(not-bound-error 'sym (current-frame))",
		exp:     "{[{symbol \"sym\" not bound in \"not-bound-error-sym-env\"->\"vars\"->\"root\"}]}",
		withErr: true,
	},
	{name: "not-bound-error-sym-env-parent",
		src:     "(not-bound-error 'sym (parent-frame (current-frame)))",
