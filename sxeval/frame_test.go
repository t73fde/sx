...
func TestFrameAlist(t *testing.T) {
	...
	frame.Bind(sx.MakeSymbol("sym4"), sx.MakeString("sym4"))
	frame.Bind(sx.MakeSymbol("sym5"), sx.MakeString("sym5"))
	frame.Bind(sx.MakeSymbol("sym6"), sx.MakeString("sym6"))
	frame.Bind(sx.MakeSymbol("sym7"), sx.MakeString("sym7"))
	alist := frame.Bindings()
	if alist.Length() != 7 {
